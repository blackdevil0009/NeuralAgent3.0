...
class _BookingBottomSheetState extends State<BookingBottomSheet> {
  ...
  void _handlePaymentSuccess(PaymentSuccessResponse response) async {
    if (_pendingAppointmentId == null) return;
    try {
  ...
  void _handlePaymentError(PaymentFailureResponse response) {
    setState(() => _isSubmitting = false);
    ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Payment failed: ${response.message}'), backgroundColor: AppColors.errorRed));
  }

  void _handleExternalWallet(ExternalWalletResponse response) {
    setState(() => _isSubmitting = false);
    ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('External Wallet: ${response.walletName}'), backgroundColor: AppColors.primaryGreen));
  }
  ...
  void _book() async {
    ...
    try {
      ...
      _pendingAppointmentId = data['appointmentId'];
      final orderId = data['orderId'];
      _pendingOrderId = orderId;
      final amount = data['amount']; // Amount in paise
