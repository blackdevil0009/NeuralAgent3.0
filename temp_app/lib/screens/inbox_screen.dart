                ...
                : RefreshIndicator(
                    ...
                    child: ListView.separated(
                      ...
                      itemBuilder: (ctx, i) {
                        ...
                        return InkWell(
                          onTap: () => Navigator.of(context).push(MaterialPageRoute(builder: (_) => PatientChatScreen(convo: c))),
                          child: Container(
...
class PatientChatScreen extends StatefulWidget {
  ...
  const PatientChatScreen({required this.convo, super.key});
  @override
...
class _PatientChatScreenState extends State<PatientChatScreen> {
  ...
  Future<void> _send({String? quickText}) async {
    ...
    final optimistic = {'id': 'tmp-${DateTime.now().millisecondsSinceEpoch}', 'sender': 'me', 'text': text, 'time': TimeOfDay.now().format(context), 'read': false, 'sending': true, 'type': 'text'};
    ...
    try {
      ...
      setState(() {
        _msgs.removeLast();
        _msgs.add({'id': res.data?['data']?['id']?.toString() ?? optimistic['id']!, 'sender': 'me', 'text': text, 'time': optimistic['time']!, 'read': false, 'type': 'text'});
        _sending = false;
      });
    } catch (_) {
                            ...
                            if (isAttachment)
                              GestureDetector(
                                ...
                                child: Container(
                                  ...
                                  decoration: BoxDecoration(
                                    color: isMe ? Colors.white.withValues(alpha: 0.15) : AppColors.primaryGreen.withValues(alpha: 0.08),
                                  ...
                                  child: isImage
                                      ...
                                      : Padding(
                                          ...
                                          child: Row(
                                            ...
                                            children: [
                                              Icon(isVideo ? Icons.video_file_rounded : Icons.picture_as_pdf_rounded, color: isMe ? Colors.white : AppColors.primaryGreen, size: 32),
                                              const SizedBox(width: 10),
                                              Expanded(child: Column(
                                                crossAxisAlignment: CrossAxisAlignment.start,
                                                children: [
                                                  Text(isVideo ? 'Video Attachment' : 'PDF Medical Report', style: TextStyle(fontFamily: 'Poppins', fontSize: 13, fontWeight: FontWeight.bold, color: isMe ? Colors.white : AppColors.textDark)),
                                                  Text('Tap to open & view', style: TextStyle(fontFamily: 'Poppins', fontSize: 11, color: isMe ? Colors.white70 : AppColors.textMuted)),
                                                ],
                                              )),
                                              Icon(Icons.download_rounded, color: isMe ? Colors.white : AppColors.primaryGreen, size: 20),
                                            ],
                                          ),
                                        ),
                                ),
