...
class _PatientAiScreenState extends State<PatientAiScreen> {
  ...
  void _sendMessage([String? prompt]) async {
    ...
    try {
      final res = await ApiClient.post(ApiConfig.aiQuery, {'message': text});
      ...
      setState(() {
        ...
        if (res.ok && res.data != null) {
          final reply = res.data!['data']?['response'] ?? 'I could not process that request.';
          _messages.insert(0, {
            'isUser': false,
            'text': reply,
            'time': 'Just now',
          ...
          _messages.insert(0, {
            'isUser': false,
            'text': res.data?['message'] ?? res.error ?? 'Sorry, I am having trouble connecting to the server.',
            'time': 'Just now',
            'isError': true,
          });
        }
          ...
          Expanded(
            child: ListView.builder(
              ...
              itemBuilder: (context, index) {
                ...
                return _buildMessageBubble(msg['text'], msg['isUser'], msg['isLoading'] == true, msg['isError'] == true);
              },
            ),
          ),
          _buildQuickPrompts(),
  ...
  Widget _buildMessageBubble(String text, bool isUser, [bool isLoading = false, bool isError = false]) {
    return Align(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          border: isUser ? null : Border.all(color: isError ? const Color(0xFFFEB2B2) : AppColors.inputBorder.withValues(alpha: 0.5)),
          boxShadow: [
        ...
        child: isLoading
            ...
            : Text(
                ...
                style: TextStyle(
                  fontFamily: 'Poppins', 
                  fontSize: 13, 
                  color: isUser ? Colors.white : (isError ? const Color(0xFFC53030) : AppColors.textDark), 
                  height: 1.4
                ),
              ),
      ),
