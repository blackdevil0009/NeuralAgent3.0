...
class _RegisterScreenState extends State<RegisterScreen>
    ...
    try {
      ...
      if (res.ok && res.data != null) {
        ...
        setState(() {
          ...
          if (res.data!['verification_token'] != null) {
            _verificationToken = res.data!['verification_token'];
          }
          if (res.data!['extracted_dob'] != null && _dobCtrl.text.isEmpty) {
            String extractedDob = res.data!['extracted_dob'];
            if (RegExp(r'^\d{4}-\d{2}-\d{2}$').hasMatch(extractedDob)) {
              final parts = extractedDob.split('-');
              extractedDob = '${parts[2]}/${parts[1]}/${parts[0]}';
            }
            _dobCtrl.text = extractedDob;
          }
          _docAutoFilled = true;
          _verifyingDoc = false;
        });
  ...
  Future<void> _submit() async {
    ...
    String mobileClean = _phoneCtrl.text.replaceAll(RegExp(r'\D'), '');
    if (mobileClean.length == 12 && mobileClean.startsWith('91')) {
      mobileClean = mobileClean.substring(2);
    } else if (mobileClean.length == 11 && mobileClean.startsWith('0')) {
      mobileClean = mobileClean.substring(1);
    }

    String backendDob = _dobCtrl.text.trim();
    if (RegExp(r'^\d{2}/\d{2}/\d{4}$').hasMatch(backendDob)) {
      final parts = backendDob.split('/');
      backendDob = '${parts[2]}-${parts[1]}-${parts[0]}';
    }
    ...
    try {
      ...
      if (_role == 0) {
        ...
        if (_verificationToken == null || _verificationToken!.isEmpty) {
          _snack('Please verify your document before registering.', isError: true);
          setState(() => _loading = false);
          return;
        }
        final fields = {
      ...
      if (res.ok) {
        ...
        if (res.statusCode == 409) {
          errMsg = 'An account with this email is already registered. Please login instead.';
        } else if (res.data != null && res.data!['errors'] != null) {
          try {
      ...
      if (_role == 0) ...[
        ...
        VxTextField(
          ...
          validator: (v) {
            ...
            if (!RegExp(r'^\d{2}/\d{2}/\d{4}$').hasMatch(t)) return 'Use DD/MM/YYYY format';
            return null;
          },
        ),
        ...
        VxTextField(
          ...
          validator: (v) {
            ...
            if (t.isNotEmpty && !RegExp(r'^\d{2}/\d{2}/\d{4}$').hasMatch(t))
