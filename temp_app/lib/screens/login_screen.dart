  ...
  Future<void> _handleLogin() async {
    ...
    try {
      ...
      if (res.ok && res.data != null) {
        ...
        if (inner['status'] == '2fa_required') {
          setState(() => _isLoading = false);
          Navigator.of(context).push(
            MaterialPageRoute(
              builder: (_) => OtpScreen(
                email: inner['email']?.toString() ?? emailText,
                purpose: '2fa',
              ),
            ),
          );
        ...
        // Real error from backend or network failure
        final errMsg = res.statusCode == 0
            ? '⚠️ Cannot reach server. Please check your internet connection or verify that the backend (https://api.vaidyamedx.in) is online.'
            : (res.data?['message'] ?? res.data?['error'] ?? res.error ?? 'Login failed. Check credentials.');
    ...
    try {
      ...
      if (res.ok || res.statusCode == 200) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(
          content: Text(res.data?['message'] ?? 'Failed to send reset link.'),
      ...
      if (!mounted) return;
      setState(() => _loading = false);
      // In dev, show success anyway if server unreachable
      setState(() => _sent = true);
    }
  }
